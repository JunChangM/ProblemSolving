...
A=12314;
B=12313;
A기 더 가중치가 높으니까  A => 9 , B => 8
하지만 여기서 입력받은 알파벳에 관계없이 가장 큰 가중치를 가진 값부터 큰 수를 할당하면 된다.
즉 가중치[0] * 9 + 가중치[1] * 8 + ... 식으로 더하면 가장 큰 값을 가질 수 있다.
위의 풀이 에서는 pair 값을 이용함으로써, 실제 조회해보면 어떤 알파벳이 가장치가 가장 큰지도 확인할 수 있다.
*/
